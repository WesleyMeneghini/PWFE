...
const buscarCidades =  ( json, uf ) =>{
   return json.filter( estados => estados.sigla == uf)[0]
               .cidades.map( cidades => cidades.nome);
}

const mostrarCidades = () =>{
   const estado = document.querySelector('#estado').value;
   const city = buscarCidades( cidades, estado.toUpperCase());
   // console.log(criarLinhas(city));
   $container.innerHTML = criarLinhas(city);
}

const criarLinhas = (cidades) => {
   return cidades.reduce( (acc, cidade, indice) =>  
         `${acc}
         <tr class="${( indice % 2 ) == 0 ? 'corLinha' : '' }">
            <td>${indice+1}</td>
            <td>${cidade}</td>
         </tr>`
      , "" );
